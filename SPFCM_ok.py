...
class SPFCM:
    ...
    def Run(self):
        ...
        final_V = self.iter_all(last_data_length,remind_data,first_U,first_V,W)
        ...
        for j in range(self.c):
            ...
            for i in range(self.dim):
                ...
                for k in range(length):
                    dummy_sum_num += (U[k][j] ** self.m) * data[k][i] * W[k]

                    dummy_sum_dum += (U[k][j] ** self.m) * W[k]
        ...
        for j in range(self.c):
            for i in range(length):
                ...
                for k in range(self.c):
                    if distance_matrix[i][k] == 0:#如果是中心，则把隶属度设为1，其余为0
    ...
    def end_conditon(self,U, U_old):
        ...
        for i in range(0, len(U)):
            for j in range(0, len(U[0])):
                if abs(U[i][j] - U_old[i][j]) > self.eps:
                    return False
        return True
    def calculate_WFCM(self,data,W,V):
        U = self.Initial_U(data)
        if V == []:
            V = self.calculate_V(U,data,W)
            iter = 1
        else:
            iter = 0
        while True:
            ...
            for i in range(self.c):
                if np.power(self.distance(V[i],V_old[i]),2) > self.eps:
                    flag = 1
                    break
            if flag == 0:
                break
        ...
        for i in range(0, len(U)):
            for j in range(0, len(U[0])):
                if abs(U[i][j] - U_old[i][j]) > self.eps:
                    return False
        return True
    def calculate_W(self,U,W,length):
        new_W = []
        for i in range(self.c):
            dummy_sum_num = 0.0
    ...
    def iter_all(self,last_data_length,remind_data,U,V,W):
        for i in range(1,int(self.size / self.ns)):
            print("第" + str(i) + "次采样")
            sample_data,remind_data = self.load_sample_data(remind_data)
            W = self.calculate_W(U,W,last_data_length)
            print(W)
            last_data_length = len(sample_data)
            new_data = copy.deepcopy(V)
            for i in range(last_data_length):
                new_data.append(sample_data[i])
            U,V = self.calculate_WFCM(new_data,W,V)
        print("------------------")
        return V
    ...
    def extension(self,data,V):
        ...
        for j in range(self.c):
            for i in range(self.size):
                dummy = 0.0
                for k in range(self.c):
                    dummy += (dis[i][j] / dis[i][k]) ** (2/(self.m-1))
                U[i][j] = 1 / dummy
        print("拓展到整个数据集上")
        print("硬聚类化 U")
